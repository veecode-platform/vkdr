...
class VkdrInfraCommand {
    ...
    int start(@Option(names = {"--traefik","--enable_traefik","--enable-traefik"},
              ...
              @Option(names = {"--nodeports"},
                ...
                int nodeports) throws IOException, InterruptedException {
        System.out.printf("'infra start' was called, enable_traefik=%s, http_port=%s, https_port=%s, nodeports=%s \n", enable_traefik, http_port, https_port, nodeports);
        return ShellExecutor.executeCommand("infra/start", String.valueOf(enable_traefik), String.valueOf(http_port), String.valueOf(https_port), String.valueOf(nodeports));
    }
