...
settingKong() {
  case $VKDR_ENV_KONG_MODE in
    dbless)
      ...
      cp "$META_DIR/values/kong-dbless.yaml" $VKDR_KONG_VALUES
      if [ "$VKDR_ENV_KONG_ENTERPRISE" = "true" ]; then
        VKDR_KONG_ENT_VALUES="$META_DIR/values/delta-kong-enterprise.yaml"
        # merge yq files
    ...
    standard)
      ...
      cp "$META_DIR/values/kong-standard.yaml" $VKDR_KONG_VALUES
      if [ "$VKDR_ENV_KONG_ENTERPRISE" = "true" ]; then
        VKDR_KONG_ENT_VALUES="$META_DIR/values/delta-kong-enterprise.yaml"
        # merge yq files
      ...
      VKDR_KONG_SECRET_VALUES="$META_DIR/values/delta-kong-std-dbsecrets.yaml"
...
configAdminOIDC() {
  if [ "true" = "$VKDR_ENV_KONG_ADMIN_OIDC" ]; then
    ...
    $VKDR_JQ '.' -c "$META_DIR/values/admin_gui_auth_conf" > /tmp/admin_gui_auth_conf
