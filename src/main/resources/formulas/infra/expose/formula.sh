...
# V2 paths: relative to formulas/infra/expose/
...
createServiceAccount() {
  ...
  $VKDR_KUBECTL apply -n "$EXPOSE_NAMESPACE" -f "$SHARED_DIR/super-admin-service-account/super-admin.yaml"
...
deleteServiceAccount() {
  ...
  $VKDR_KUBECTL delete -n "$EXPOSE_NAMESPACE" -f "$SHARED_DIR/super-admin-service-account/super-admin.yaml"
}
...
generateSAKubeConfig() {
  ...
  mkdir -p ~/.vkdr/tmp
  local kconfig_src="$SHARED_DIR/super-admin-service-account/kconfig.template"
